            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <select name="torre"
                        ...
                        <option value="Torre 1" {{ old('torre', $apartamento->torre) == 'Torre A' ? 'selected' : '' }}>Torre
                            A</option>
                        <option value="Torre 2" {{ old('torre', $apartamento->torre) == 'Torre B' ? 'selected' : '' }}>Torre
                            B</option>
                        <option value="Torre 3" {{ old('torre', $apartamento->torre) == 'Torre C' ? 'selected' : '' }}>Torre
                            C</option>
                    ...
                    <select name="tipo_apartamento_id"
                        ...
                        @foreach ($tipos as $tipo)
                            <option value="{{ $tipo->id }}"
                                {{ old('tipo_apartamento_id', $apartamento->tipo_apartamento_id) == $tipo->id ? 'selected' : '' }}>
                                {{ $tipo->nombre }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="propietario_id"
                        ...
                        @foreach ($propietarios as $propietario)
                            <option value="{{ $propietario->id }}"
                                {{ old('propietario_id', $apartamento->propietario_id) == $propietario->id ? 'selected' : '' }}>
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ...
                    <input type="number" name="deuda_actual" value="{{ old('deuda_actual', $apartamento->deuda_actual) }}"
                        step="0.01" min="0"
                        style="padding: 0.8rem; border-radius: 8px; border: 1px solid var(--color-borde); background: var(--color-superficie); color: var(--color-texto);">
                </div>
            </div>
